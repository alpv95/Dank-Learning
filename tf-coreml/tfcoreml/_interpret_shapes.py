...
def _matmul(op, blob_name, output_name, context):
  # Hardcode in one shape dict translation.
  shape1 = context.shape_dict[op.inputs[0].name]
  shape2 = context.shape_dict[op.inputs[1].name]
  output_shape = [shape1[0], shape2[1]]
  context.shape_dict[output_name] = output_shape

def _transpose(op, blob_name, output_name, context):
  # Hardcode in one shape dict translation.
  input_shape = context.shape_dict[op.inputs[0].name]
